...
const Repository: React.FC<IRepositoryProps> = ({repo}) => {
    return (
        <NavLink to={`/card/${repo.owner.login}/${repo.name}`} className={cl.repository}>
            <div className={cl.repository_header}>
                <div className={cl.repository_avatar}>
                    <img src={repo.owner.avatar_url} alt=""/>
                </div>
                <div className={cl.repository_info} onClick={e => e.stopPropagation()}>
                    <a href={repo.html_url} className={cl.title}>
                        {repo.full_name}
                    </a>
                    <div className={cl.dop_info}>
                        <div>Stars: {repo.stargazers_count}</div>
                        <div>Updated at: {new Date(repo.updated_at).toLocaleDateString()}</div>
                        <div>Created at: {new Date(repo.created_at).toLocaleDateString()}</div>
                        <div>Pushed at: {new Date(repo.pushed_at).toLocaleDateString()}</div>
                    </div>
                </div>
            </div>
